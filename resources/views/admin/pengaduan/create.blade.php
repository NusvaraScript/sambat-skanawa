    ...
    <section class="section">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Form Tambah Pengaduan</h4>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.pengaduan.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="siswa_nis" id="siswa_nis" class="form-select @error('siswa_nis') is-invalid @enderror" required>
                            ...
                            @foreach ($siswas as $siswa)
                                <option value="{{ $siswa->nis }}" @selected(old('siswa_nis') == $siswa->nis)>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="judul_laporan" id="judul_laporan" value="{{ old('judul_laporan') }}" class="form-control @error('judul_laporan') is-invalid @enderror" required>
                        @error('judul_laporan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="isi_laporan" id="isi_laporan" rows="5" class="form-control @error('isi_laporan') is-invalid @enderror" required>{{ old('isi_laporan') }}</textarea>
                        @error('isi_laporan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="foto" id="foto" value="{{ old('foto') }}" class="form-control @error('foto') is-invalid @enderror" placeholder="Nama file atau URL foto (opsional)">
                        @error('foto')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
